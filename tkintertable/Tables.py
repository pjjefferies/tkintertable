...
class TableCanvas(tk.Canvas):
    ...
    def __init__(self, parent=None, model=None, width=None, height=None,
        ...
        self.allrows = False  # for selected all rows without
        #                       setting multiplerowlist
        ...
        self.filterwin = None
        self.rightmenu = None
        self.formulaFrame = None
        self.formulaText = None
        self.formulaWin = None
        self.clipboard = None
        self.pyplot = None
        self.cellentry = None
        self.entrywin = None
        self.prefswindow = None
    ...
    def setFontSize(self):
        """Set font size to match font, we need to get rid of fontsize as
            a separate variable?"""
        if hasattr(self, 'thefont') and isinstance(self.thefont, tuple):
    ...
    def createTableFrame(self, callback=None):
        """Adds column header and scrollbars and combines them with
           the current table adding all to the master frame provided in
           constructor. Table is then redrawn."""
        ...
        self.Yscrollbar = AutoScrollbar(self.parentframe, orient='vertical',
                                        command=self.set_yviews)
        self.Yscrollbar.grid(row=1, column=2, rowspan=1, sticky='news',
                             pady=0, ipady=0)
        self.Xscrollbar = AutoScrollbar(self.parentframe, orient='horizontal',
    ...
    def redrawVisible(self, event=None, callback=None):
        ...
        if self.filtered and self.model.filteredrecs is not None:
        ...
        self.visiblerows = list(range(int(startvisiblerow),
                                      int(endvisiblerow)))
        startvisiblecol, endvisiblecol = self.getVisibleCols(x1, x2)
        self.visiblecols = list(range(int(startvisiblecol),
                                      int(endvisiblecol)))
        ...
        self.tablerowheader.redraw(align=self.align,
                                   showkeys=self.showkeynamesinheader)
    ...
    def redrawCell(self, row=None, col=None, recname=None, colname=None):
        ...
        if row is None and recname is not None:
            row = self.model.getRecordIndex(recname)
        if col is None and colname is not None:
    ...
    def addRows(self, num=None):
        """Add new rows"""
        if num is None:
            num = tk.simpledialog.askinteger("Now many rows?",
                                             "Number of rows:",
                                             initialvalue=1,
    ...
    def deleteCells(self, rows, cols):
        ...
        n = tk.messagebox.askyesno("Clear Confirm",
                                   "Clear this data?",
                                   parent=self.parentframe)
        if not n:
            return
    ...
    def autoAddColumns(self, numcols=None):
        ...
        if numcols is None:
            numcols = tk.simpledialog.askinteger("Auto add rows.",
    ...
    def getRecordInfo(self, row):
        ...
        RecordViewDialog(title="Record Details",
                         parent=self.parentframe, table=self, row=row)
        return
    ...
    def findValue(self, searchstring=None, findagain=None):
        ...
        if searchstring is None:
            searchstring = tk.simpledialog.askstring("Search table.",
                                                     "Enter search value",
                                                     parent=self.parentframe)
        found = 0
        if findagain is None or not hasattr(self, 'foundlist'):
            self.foundlist = []
            ...
            for row in range(self.rows):
                for col in range(self.cols):
                    ...
                    if text.lower().find(searchstring.lower()) != -1:
                        ...
                        self.drawRect(row, col, color='red',
                                      tag='searchrect', delete=0)
    ...
    def createFilteringBar(self, parent=None, fields=None):
        ...
        if parent is None:
            ...
            x, y, _, h = self.getGeometry(self.master)
    ...
    def get_col_clicked(self, event):
        ...
        # x_start = self.x_start    # originally was not commented-out
        # print self.col_positions
        for colpos in self.col_positions:
            try:
                nextpos = self.col_positions[self.col_positions.index(colpos) +
                                             1]
            except (AttributeError, IndexError):
                nextpos = self.tablewidth
            if x > colpos and x <= nextpos:
                # print 'x=', x, 'colpos', colpos,
                # self.col_positions.index(colpos)
    ...
    def getCanvasPos(self, row, col):
        ...
        x1, y1, _, _ = self.getCellCoords(row, col)
    ...
    def isInsideTable(self, x, y):
        ...
        if (self.x_start < x < self.tablewidth and
                self.y_start < y < self.rows * self.rowheight):
            return 1
        else:
            return 0
        # return answer
    ...
    def handle_left_click(self, event):
        ...
        if hasattr(self, 'cellentry') and self.cellentry is not None:
            self.cellentry.destroy()
        # ensure popup menus are removed if present
        # if hasattr(self, 'rightmenu'):
        if hasattr(self, 'rightmenu') and self.rightmenu is not None:
            self.rightmenu.destroy()
        if (hasattr(self.tablecolheader, 'rightmenu') and
                self.rightmenu is not None):
    ...
    def handle_mouse_drag(self, event):
        ...
        if colover > self.cols or self.startcol > self.cols:
            ...
            if self.endcol < self.startcol:
                self.multiplecollist = list(range(self.endcol,
                                                  self.startcol + 1))
            else:
                self.multiplecollist = list(range(self.startcol,
                                                  self.endcol + 1))
            # print self.multiplecollist
        # draw the selected rows
        if self.endrow != self.startrow:
            if self.endrow < self.startrow:
                self.multiplerowlist = list(range(self.endrow,
                                                  self.startrow + 1))
            else:
                self.multiplerowlist = list(range(self.startrow,
                                                  self.endrow + 1))
            self.drawMultipleRows(self.multiplerowlist)
            self.tablerowheader.drawSelectedRows(self.multiplerowlist)
    ...
    def handle_arrow_keys(self, event):
        ...
        # print event.keysym
        # row = self.get_row_clicked(event)  # not originally commented
        # col = self.get_col_clicked(event)  # not originally commented
        # x, y = self.getCanvasPos(self.currentrow, 0)
        x, _ = self.getCanvasPos(self.currentrow, 0)
        if x is None:
            return
    ...
    def handle_right_click(self, event):
        ...
        if ((rowclicked in self.multiplerowlist or self.allrows) and
                (colclicked in self.multiplecollist)):
    ...
    def formula_Dialog(self, row, col, currformula=None):
        ...
        self.formulaFrame = tk.Frame(width=w, height=h, bd=3, relief='ridge')
        self.formulaText = tk.Text(self.formulaFrame, width=30, height=8,
                                   bg='white', relief='groove')
        self.formulaText.pack(side='left', padx=2, pady=2)
        if currformula is not None:
            self.formulaText.insert('end', Formula.getFormula(currformula))
        cancelbutton = tk.Button(self.formulaFrame, text='Cancel',
                                 relief='groove', bg='#99ccff', command=close)
        cancelbutton.pack(fill='both', padx=2, pady=2)
        donebutton = tk.Button(self.formulaFrame, text='Done', relief='groove',
        ...
        # add to canvas
        self.formulaWin = self.create_window(x1 + self.inset,
                                             y1 + self.inset,
                                             width=w, height=h,
                                             window=self.formulaFrame,
                                             anchor='nw',
                                             tag='formulabox')
        self.formulaText.focus_set()
    ...
    def setcellColor(self, rows, cols=None, newColor=None,
                     key=None, redraw=True):
        """Set the cell color for one or more cells and save it in
        the model color"""
        ...
        if newColor is None:
            ...
            _, newColor = askcolor(title='pick a color')
            if newColor is None:
                return
    ...
    def popupMenu(self, event, rows=None, cols=None, outside=None):
        """Add left and right click behaviour for canvas, should not have to
           override this function, it will take its values from defined dicts
           in constructor"""
        ...
        main = ["Set Fill Color", "Set Text Color", "Copy", "Paste",
                "Fill Down", "Fill Right", "Clear Data"]
        general = ["Select All", "Add Row(s)", "Delete Row(s)",
                   "Auto Fit Columns", "Filter Records", "Preferences"]
        ...
        if outside is None:
            ...
            def add_defaultcommands():
                ...
                for action in main:
                    if (action == 'Fill Down' and
                            (rows is None or len(rows) <= 1)):
                        continue
                    if (action == 'Fill Right' and
                            (cols is None or len(cols) <= 1)):
                        continue
                    else:
    ...
    def fillDown(self, rowlist, collist):
        ...
        row = rowlist[0]  # Added to hopefully make line 5 down work
        ...
        for col in collist:
            ...
            # i = 1
            for i, r in enumerate(rowlist, 1):
                # absr = self.get_AbsoluteRow(r)
    ...
    def fillAcross(self, collist, rowlist):
        ...
        for row in rowlist:
            # absr = self.get_AbsoluteRow(row)  # was commented out but req'd?
            # val = self.model.getCellRecord(absr, frstcol)  # original
            val = self.model.getCellRecord(row, frstcol)
            f = val     # formula to copy
            # i = 1
            for i, c in enumerate(collist):
                if Formula.isFormula(f):
                    # newval = model.copyFormula(f, r, c, offset=i, dim='x')
                    newval = model.copyFormula(f, row, c, offset=i, dim='x')
                    # model.setFormulaAt(newval, r, c)
                    model.setFormulaAt(newval, row, c)
                else:
                    # model.setValueAt(val, r, c)
                    model.setValueAt(val, row, c)
                # i += 1
        self.redrawTable()
        return
    ...
    def plotSetup(self):
        """Call pylab plot dialog setup, send data if we haven't already
            plotted"""
        # from PylabPlot import pylabPlotter  # original line
        from Plot import pylabPlotter
        if not hasattr(self, 'pyplot'):
            self.pyplot = pylabPlotter()
        plotdata = self.getSelectionValues()
        if not self.pyplot.hasData() and plotdata is not None:
            print('has data')
            plotdata = self.getSelectionValues()
            pltlabels = self.getplotlabels()
            self.pyplot.setDataSeries(pltlabels)
    ...
    def drawGrid(self, startrow, endrow):
        ...
        cols = self.cols
        # w = self.cellwidth  # Not originally commented-out
        h = self.rowheight
        x_start = self.x_start
        y_start = self.y_start
        # x_pos = x_start  # Not originally commented-out

        if self.vertlines == 1:
            for col in range(cols+1):
                x = self.col_positions[col]
                self.create_line(x, y_start, x, y_start + rows * h,
                                 tag='gridline', fill=self.grid_color,
                                 width=self.linewidth)
        if self.horizlines == 1:
            for row in range(int(startrow), int(endrow + 1)):
                y_pos = y_start + row * h
                self.create_line(x_start, y_pos, self.tablewidth, y_pos,
                                 tag='gridline', fill=self.grid_color,
                                 width=self.linewidth)
        return
    ...
    def drawRowHeader(self):
        ...
        x_start = self.x_start
        # y_start = self.y_start  # Not originally commented-out
        h = self.rowheight
        # rowpos = 0
        for rowpos, row in enumerate(self.rowrange):
            # x1, y1, x2, y2 = self.getCellCoords(rowpos, 0)
            _, y1, _, y2 = self.getCellCoords(rowpos, 0)
    ...
    def drawSelectedRect(self, row, col, color=None):
        ...
        w = 3
        x1, y1, x2, y2 = self.getCellCoords(row, col)
        # rect = self.create_rectangle(x1 + w / 2,
        #                              y1 + w / 2,
        #                              x2 - w / 2,
        #                              y2 - w / 2,
        #                              fill=bg,
    ...
    def drawRect(self, row, col, color=None, tag=None, delete=1):
        ...
        w = 1
        x1, y1, x2, y2 = self.getCellCoords(row, col)
        # rect = self.create_rectangle(x1 + w / 2, y1 + w / 2,
        #                              x2 - w / 2, y2 -w / 2,
        #                              fill=bg,
        #                              outline=bg,
        #                              width=w,
        #                              tag=(recttag, 'cellbg' +
        #                                   str(row) + str(col)))
        self.create_rectangle(x1 + w / 2, y1 + w / 2,
                              x2 - w / 2, y2 - w / 2,
                              fill=bg,
                              outline=bg,
                              width=w,
    ...
    def drawCellEntry(self, row, col, text=None):
        ...
        if Formula.isFormula(cellvalue):
            ...
            text = self.model.getValueAt(row, col)
        # x1, y1, x2, y2 = self.getCellCoords(row, col)
        x1, y1, x2, _ = self.getCellCoords(row, col)
        w = x2 - x1
        ...
        self.cellentry = tk.Entry(self.parentframe,
                                  width=20,
                                  textvariable=txtvar,
                                  bg=self.entrybackgr,
                                  relief='flat',
    ...
    def drawText(self, row, col, celltxt, fgcolor=None, align=None):
        ...
        if w < 18:
            ...
            fontsize = self.fontsize
            # colname = self.model.getColumnName(col)  # Not orig. commented
    ...
    def drawMultipleCells(self):
        ...
        x1, y1, _, _ = self.getCellCoords(rows[0], cols[0])
        # c, d, x2, y2 = self.getCellCoords(rows[len(rows)-1],
        #                                   cols[len(cols)-1])
        _, _, x2, y2 = self.getCellCoords(rows[len(rows) - 1],
                                          cols[len(cols) - 1])
        # rect = self.create_rectangle(x1 + w / 2, y1 + w / 2,
        #                              x2, y2,
        #                              outline='blue',
    ...
    def drawTooltip(self, row, col):
        ...
        # try added by daniel because it goes wrong sometimes in the
        # corpkit gui
        try:
            sfont = {'font': tk.font.Font(family='Arial', size=12,
                                          weight='bold')}
        except AttributeError:
            sfont = {}
        ...
        # rect = self.create_rectangle(x1 + 1, y1 + 1,
        #                              x2 + 1,y2 + 1,
        #                              tag='tooltip',
        #                              fill='black')
        self.create_rectangle(x1 + 1, y1 + 1,
                              x2 + 1, y2 + 1,
                              tag='tooltip',
                              fill='black')
        # rect2 = self.create_rectangle(x1, y1,
        #                               x2, y2,
        #                               tag='tooltip',
        #                               fill='lightyellow')
    ...
    def showtablePrefs(self, prefs=None):
        ...
        if self.prefs is None:
            self.loadPrefs()
        self.prefswindow = tk.Toplevel()
        x, y, w, h = self.getGeometry(self.master)
        # self.prefswindow.geometry('+%s+%s' % (x + w / 2, y + h / 2))
        self.prefswindow.geometry('+{:d}+{:d}'.format(int(x + w / 2),
                                                      int(y + h / 2)))
        self.prefswindow.title('Preferences')
        self.prefswindow.resizable(width=False, height=False)

        frame1 = tk.Frame(self.prefswindow)
        frame1.pack(side='left')
        frame2 = tk.Frame(self.prefswindow)
        frame2.pack()
        ...
        tk.Checkbutton(frame1,
                       ...
                       variable=self.horizlinesvar,
                       onvalue=1,
                       offvalue=0).grid(row=row, column=0,
                                        columnspan=2, sticky='news')
        row += 1
        tk.Checkbutton(frame1,
                       text="Show vertical lines",
                       variable=self.vertlinesvar,
                       onvalue=1,
                       offvalue=0).grid(row=row, column=0,
                                        columnspan=2, sticky='news')
        row += 1
        ...
        self.fontbox.grid(row=row, column=0, columnspan=2, sticky='nes',
                          padx=3, pady=2)
        row += 1

        lblfontsize = tk.Label(frame2, text='Text Size:')
        lblfontsize.grid(row=row, column=0, padx=3, pady=2)
        fontsizeentry = tk.Scale(frame2, from_=6, to=50, resolution=1,
                                 orient='horizontal',
                                 relief='ridge',
        ...
        # cell alignment
        lbl = tk.Label(frame2, text='Alignment:')
        lbl.grid(row=row, column=0, padx=3, pady=2)
        alignentry_button = tk.Menubutton(frame2,
                                          textvariable=self.cellalignvar,
                                          relief='raised',
                                          width=16)
        alignentry_menu = tk.Menu(alignentry_button, tearoff=False)
        ...
        # colors
        cellbackgrbutton = tk.Button(frame2,
        ...
        cellbackgrbutton.grid(row=row, column=0, columnspan=2, sticky='news',
                              padx=3, pady=2)
        row += 1
        grid_colorbutton = tk.Button(frame2,
                                     text='grid color',
                                     bg=self.grid_color,
                                     foreground='black',
        ...
        grid_colorbutton.grid(row=row, column=0, columnspan=2, sticky='news',
                              padx=3, pady=2)
        row += 1
        rowselectedcolorbutton = tk.Button(frame2,
                                           text='row highlight color',
                                           bg=self.rowselectedcolor,
                                           foreground='black',
                                           highlightcolor='white',
                                           relief='groove',
                                           command=self.setrowselectedcolor)
        rowselectedcolorbutton.grid(row=row, column=0, columnspan=2,
                                    sticky='news', padx=3, pady=2)
        row += 1

        frame = tk.Frame(self.prefswindow)
        frame.pack()
        # Apply Button
        b = tk.Button(frame, text="Apply Settings", command=self.applyPrefs)
    ...
    def getFonts(self):
        fonts = set(list(tk.font.families()))
    ...
    def loadPrefs(self, prefs=None):
        ...
        self.prefs = prefs
        defaultprefs = {
            'horizlines': self.horizlines, 'vertlines': self.vertlines,
        ...
        self.thefont = (self.prefs.get('celltextfont'),
                        self.prefs.get('celltextsize'))
        self.rowheaderwidthvar = tk.IntVar()
    ...
    def AskForColorButton(self, frame, text, func):
        def SetColor():
            ...
            _, _ = askcolor(title='pick a color',
                            initialcolor=self.cellbackgr)
            return

        bgcolorbutton = tk.Button(frame, text=text, command=SetColor)
    ...
    def checkOSType(cls):
        ...
        if ostyp.find('windows') != -1:
            ...
            try:
                # pylint: disable=E1101
                info = os.uname()   # for unix flavors only
            except AttributeError:
                pass
            ostyp = info[0].lower()
...
class ColumnHeader(tk.Canvas):
    ...
    def __init__(self, parent=None, table=None):
        tk.Canvas.__init__(self, parent, bg='gray25', width=500, height=20)
        ...
        if table is not None:
            ...
            self.rightmenu = None  # added for pylint
            self.draggedcol = None  # added for pylint
            self.atdivider = None  # added for pylint
            self.tablewidth = None  # added for pylint
        return
    ...
    def redraw(self):
        ...
        self.configure(scrollregion=(
            0, 0, self.table.tablewidth + self.table.x_start, self.height))
        self.delete('gridline', 'text')
        self.delete('rect')
        self.atdivider = None

        height = self.height
        # x_start = self.table.x_start  # Not originally commented-out
        if cols == 0:
            return
        for col in self.table.visiblecols:
            ...
            if colname in self.model.columnwidths:
                width = self.model.columnwidths[colname]
            else:
                width = self.table.cellwidth
            x = self.table.col_positions[col]

            if len(collabel) > width / 10:
                collabel = collabel[0:int(int(width) / 12)] + '.'
            # line = self.create_line(x, 0, x, height,
            self.create_line(x, 0, x, height,
                             tag=('gridline', 'vertline'),
                             fill='white', width=2)

            self.create_text(x + width / 2, height / 2,
                             text=collabel,
                             fill='white',
                             font=self.thefont,
                             tag='text')
        ...
        self.create_line(x, 0, x, height, tag='gridline',
                         fill='white', width=2)
    ...
    def handle_left_release(self, event):
        ...
        if self.atdivider == 1:
            ...
            x1, _, _, _ = self.table.getCellCoords(0, col)
            newwidth = x - x1
            if newwidth < 5:
                newwidth = 5
        ...
        self.delete('resizesymbol')
        # move column
        if (self.draggedcol is not None and
                self.table.currentcol != self.draggedcol):
    ...
    def handle_mouse_drag(self, event):
        ...
        if self.atdivider == 1:
            ...
            x1, _, _, _ = self.coords('dragrect')
            x = int(self.canvasx(event.x))
            # y = self.canvasy(event.y)  # not originally commented-out
    ...
    def handle_mouse_move(self, event):
        ...
        if self.table.cellwidth is None or self.tablewidth is None:
            return
        self.delete('resizesymbol')
        width = self.table.cellwidth
        # h = self.height  # not originally commented-out
        x_start = self.table.x_start
        # x = event.x
        x = int(self.canvasx(event.x))
        if x is None:
            return
        if x > self.tablewidth + width:
            return
    ...
    def popupMenu(self, event):
        ...
        collabel = self.model.columnlabels[colname]
        currcol = self.table.currentcol
        popupmenu = tk.Menu(self, tearoff=False)

        def popupFocusOut(event):
            popupmenu.unpost()

        popupmenu.add_command(label="Rename Column",
                              command=self.relabel_Column)
        popupmenu.add_command(label="Sort by " + collabel,
                              command=lambda: self.table.sortTable(currcol))
        popupmenu.add_command(label="Sort by " + collabel + ' (descending)',
                              command=lambda: self.table.sortTable(currcol,
                                                                   reverse=1))
        popupmenu.add_command(label="Delete This Column",
                              command=self.table.deleteColumn)
        popupmenu.add_command(label="Add New Column",
    ...
    def relabel_Column(self):
        col = self.table.currentcol
        ans = tkinter.simpledialog.askstring('New column name?',
                                             "Enter new name:")
        if ans is not None:
            if ans == '':
    ...
    def draw_resize_symbol(self, col):
        ...
        self.delete('resizesymbol')
        # w = self.table.cellwidth  # not originally commented-out
        h = self.height
        # if x_pos > self.tablewidth:
        #    return
        wdth = 1
        # hfac1 = 0.2  # not originally commented-out
        # hfac2 = 0.4  # not originally commented-out
        # x_start = self.table.x_start  # not originally commented-out
        # x1, y1, x2, y2 = self.table.getCellCoords(0, col)
        _, _, x2, _ = self.table.getCellCoords(0, col)

        self.create_polygon(x2 - 3, h / 4, x2 - 10, h / 2, x2 - 3, h * 3 / 4,
                            tag='resizesymbol', fill='white', outline='gray',
                            width=wdth)
        self.create_polygon(x2 + 2, h / 4, x2 + 10, h / 2, x2 + 2, h * 3 / 4,
                            tag='resizesymbol', fill='white', outline='gray',
                            width=wdth)
        return
    ...
    def drawRect(self, col, tag=None, color=None, outline=None, delete=1):
        ...
        # rect = self.create_rectangle(x1, y1 - w,
        #                              x2, self.height,
        #                              fill=color,
        #                              outline=outline,
        #                              width=w,
...
class RowHeader(tk.Canvas):
    ...
    def __init__(self, parent=None, table=None, width=40):
        tk.Canvas.__init__(self, parent, bg='gray75', width=width, height=None)
    ...
    def handle_mouse_drag(self, event):
        ...
        if hasattr(self, 'cellentry'):
            ...
            self.cellentry.destroy()
        rowover = self.table.get_row_clicked(event)
        # colover = self.table.get_col_clicked(event)  # not orig. commented
        if rowover is None:
            return
    ...
    def drawRect(self, row=None, tag=None, color=None, outline=None, delete=1):
        ...
        #                              width=w,
        #                              tag=tag)
        self.create_rectangle(0 + i, y1 + i,
                              self.width - i, y2,
                              fill=color,
                              outline=outline,
                              width=w,
