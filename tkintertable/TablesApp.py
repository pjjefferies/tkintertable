...
import Pmw
# import tk.filedialog, tk.messagebox, tk.simpledialog
# import re
import os
# import time
import pickle
...
class TablesApp(tk.Frame):
    ...
    def __init__(self,
        ...
        if not self.parent:
            ...
            self.tablesapp_win = tk.Toplevel()
    ...
    def createMenuBar(self):
        ...
        self.menu = tk.Menu(self.tablesapp_win)
    ...
    def create_pulldown(self, menu, a_dict):
        ...
        for item in items:
            if item[-3:] == 'sep':
                ...
                command = None
                if 'cmd' in a_dict[item]:
                    command = a_dict[item]['cmd']

                # Put the command in there
                if 'sc' in a_dict[item]:
                    var.add_command(label='%-25s %9s' % (item[2:],
                                                         a_dict[item]['sc']),
                                    command=command)
                else:
    ...
    def createSearchBar(self, event=None):
        ...
        frame = tk.Frame(self.tablesapp_win)
        row = 0
        ...
        self.findtext = tk.StringVar()
        self.findbox = tk.Entry(frame,
        ...
        tk.Label(frame, text='Find:').grid(row=row, column=0, sticky='ew')
        self.findagainbutton = tk.Button(frame, text='Find Again',
                                         command=self.do_find_again)
        self.findagainbutton.grid(row=row, column=3,
                                  sticky='news', padx=2, pady=2)
        self.cbutton = tk.Button(frame, text='Close', command=close)
    ...
    def do_save_project(self, filename):
        ...
        for sheet in self.sheets:
            currtable = self.sheets[sheet]
            model = currtable.getModel()
            data[sheet] = model.getData()
    ...
    def rename_Sheet(self):
        """Rename a sheet"""
        s = self.notebook.getcurselection()
        newname = tk.simpledialog.askstring('New sheet name?',
    ...
    def about_Tables(self):
        self.ab_win = tk.Toplevel()
        self.ab_win.geometry('+100+350')
        self.ab_win.title('About TablesApp')

        import Table_images
        logo = Table_images.tableapp_logo()
        label = tk.Label(self.ab_win, image=logo)
        label.image = logo
        label.grid(row=0, column=0, sticky='news', padx=4, pady=4)

        text = ['Tables Sample App ', 'Shows the use of Tablecanvas class ' +
                'for tkinter',
                'Copyright (C) Damien Farrell 2008', 'This program is free ' +
        ...
        row = 1
        for line in text:
            tmp = tk.Label(self.ab_win, text=line)
...
class ToolBar(tk.Frame):
    ...
    def __init__(self, parent=None, parentapp=None):
        tk.Frame.__init__(self, parent, width=600, height=40)
        import Table_images
    ...
    def add_button(self, name, callback, img=None):
        if img is None:
            b = tk.Button(self, text=name, command=callback,
                          relief='groove')
        else:
            b = tk.Button(self, text=name, command=callback,
                          relief='groove',
